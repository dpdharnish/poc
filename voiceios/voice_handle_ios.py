...
#terminal command: python3 voice_handle_ios.py -id 00008020-001E71D10168002E -url https://dev-in-cjb-0.headspin.io:7003/v0/150f14a11db946ffb9505e3175ae9d95/wd/hub
# note :  pealse give device_address and hostname manually
...
voice_btn = '(//XCUIElementTypeButton[@name="GMOEIDVoiceSearchButton"])[2]'
message = '//XCUIElementTypeStaticText[@name="GMOTitleViewLabelEID"]'
device_address = '00008020-001E71D10168002E@dev-in-cjb-0-proxy-3-mac.headspin.io'
hostname = 'dev-in-cjb-0-proxy-3-mac.headspin.io'
...
def prepare_audio(api_key, hostname, audio_ids):
    ...
    try:
        ...
        if response.status_code == 200:
            print("audio preparing done was successful")
...
def start_audio_injection(api_key, device_address, audio_id):
    ...
    try:
        ...
        response = requests.post(url, headers=headers, json=data)
        if response.status_code == 200:
            print("injection is done upload was successful")
...
try:
    ...
    try:
        ...
        print("Google Search Not converting the voice commmand to text...")
finally:
    sleep(5)
    driver.quit()